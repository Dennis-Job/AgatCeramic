...
Возвращает текущую анонимную корзину. Если заголовок `X-Cart-Token` отсутствует, API создаёт новую
корзину и возвращает её `data.identifier` (64-символьный hex-токен). Клиент сохраняет этот
идентификатор и передаёт его в том же заголовке при следующих запросах. Неизвестный идентификатор
