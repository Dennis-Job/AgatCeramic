...
Оформляет заказ из текущей корзины. Требует `X-Cart-Token`, ФИО, телефон и адрес доставки; email и
комментарий необязательны. Поле `website` — honeypot и запрещено для обычного клиента. Запросы с одного
IP ограничены пятью в минуту. Сервер в одной транзакции заново проверяет активность товара, цену и
