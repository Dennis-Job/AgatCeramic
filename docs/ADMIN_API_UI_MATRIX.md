...
| --- | --- | --- | --- |
| `GET /admin/order-statuses`; `PATCH /admin/orders/{order}/status`; `PATCH /admin/orders/{order}/payment`; `GET /admin/orders/{order}/status-history`; `GET`, `POST /admin/orders/{order}/comments` | `orders.view`, `orders.manage`, `payments.manage` according to action | `/orders` is `PlaceholderView`; it has no service calls, loading/empty/error/forbidden states, and the sidebar does not gate its link with `orders.view`. There is also no `GET /admin/orders` or `GET /admin/orders/{order}` API operation, so a manager cannot select an order safely. | **TASK-A018**: add permission-safe list/show API and an order list/detail workspace; gate navigation/route by `orders.view`, render forbidden state, and scope mutation controls to `orders.manage` / `payments.manage`. |
| `GET /admin/contact-statuses`; `GET /admin/contact-requests`; `GET /admin/contact-requests/{contactRequest}`; `PATCH .../assignee`; `PATCH .../status`; `GET .../status-history`; `GET`, `POST .../comments` | `contacts.view`, `contacts.manage` according to action | No `/contacts` route, navigation item, service or component exists. All operations are backend-ready but have no loading/empty/error/forbidden UI. | **TASK-A019**: add permission-safe contacts navigation and a list/detail workspace with filters, assignment, status workflow, history and comments; gate route/navigation by `contacts.view` and controls by `contacts.manage`. |
