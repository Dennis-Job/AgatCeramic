...
Файлы `*_AUDIT.md` и явно названные baseline/recovery reports сохраняют evidence на дату
проверки. В начале такого файла указываются дата, исторический статус и ссылки на актуальные
`CURRENT_STATE.md` и `CI.md`. Число маршрутов, файлов, тестов и assertions в историческом отчёте
не обновляется после завершения аудита и не используется как текущая метрика.

Новые правила, ограничения и команды переносятся в соответствующий канонический документ.
Исторический report может получить status-аннотацию о закрытии finding, но не должен превращаться
во второй источник текущего состояния.
...
Внутренние Markdown-ссылки и отсутствие запрещённых tracked artifacts проверяются локально и в CI:

```bash
python3 scripts/check-repository-hygiene.py
```

Проверка отклоняет несуществующие и выходящие за пределы репозитория относительные ссылки,
несуществующие Markdown anchors, а также tracked dependency/build/test/editor/runtime outputs.
Database dumps дополнительно проверяются по всем Git refs отдельным security guard.
