...
При checkout с указанным email после commit отправляется задача `SendOrderConfirmation`. Она загружает
неизменяемые snapshots заказа и позиций и отправляет письмо с номером, составом и итогом. Пустой email
