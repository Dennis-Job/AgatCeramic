...
| Admin refactoring | TASK-A030 | Историческая приёмка от 2026-09-12 отозвана повторным аудитом: выявлены blocking findings по mobile sidebar, контрасту/tokens, products/auth migration и устойчивости acceptance suite. Задача возвращена в TODO и заблокирована до `TASK-A031`–`TASK-A035`. См. [`ADMIN_REFACTORING_BASELINE.md`](../docs/ADMIN_REFACTORING_BASELINE.md). |

## Проверки последней приёмки

Повторный аудит `TASK-A030` от 2026-09-12 выявил blocking findings, которые прежний набор
проверок не обнаружил. Повторная приёмка выполняется только после `TASK-A031`–`TASK-A035`.
