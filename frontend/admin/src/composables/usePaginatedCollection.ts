...
type PageLoader<T, TResult extends PaginatedResponse<T>> = (
  page: number,
) => Promise<TResult>
...
export type PaginatedCollection<T> = {
  ...
  error: Ref<string>
  loading: Ref<boolean>
  load: <TResult extends PaginatedResponse<T>>(
    page: number,
    loader: PageLoader<T, TResult>,
  ) => Promise<TResult | null>
  reloadAfterDeletion: <TResult extends PaginatedResponse<T>>(
    loader: PageLoader<T, TResult>,
  ) => Promise<TResult | null>
}
...
export function usePaginatedCollection<T>(
  ...
  async function load<TResult extends PaginatedResponse<T>>(
    page: number,
    loader: PageLoader<T, TResult>,
  ): Promise<TResult | null> {
  ...
  async function reloadAfterDeletion<TResult extends PaginatedResponse<T>>(
    loader: PageLoader<T, TResult>,
  ): Promise<TResult | null> {
    const currentPage = pagination.value?.current_page ?? 1
    const targetPage =
      items.value.length <= 1 ? Math.max(1, currentPage - 1) : currentPage
    const response = await load(targetPage, loader)
    if (!response) return null

    const lastPage = Math.max(1, response.meta.last_page)
    const pageIsInvalid =
      response.meta.current_page > lastPage ||
      (response.data.length === 0 && response.meta.current_page > 1)
