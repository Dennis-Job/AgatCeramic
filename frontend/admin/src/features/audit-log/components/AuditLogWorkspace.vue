...
const entityName = (log: AuditLog): string =>
  log.entity?.name ??
...
<template>
  <section class="mx-auto admin-page">
    ...
    <UiCard :padded="false">
      ...
      >
        <UiInput
          v-model="audit.filters.value.search"
          searchable
          placeholder="Действие или сотрудник"
          aria-label="Поиск в журнале"
        />
        <UiSelect
          v-model="audit.filters.value.action"
          :options="auditActionOptions"
          accessible-name="Тип действия"
        />
        <UiDatePicker
          v-model="audit.filters.value.date_from"
          class="[&_[role=dialog]]:max-w-[calc(100vw-4rem)]"
          accessible-name="Дата с"
        />
        <UiDatePicker
          v-model="audit.filters.value.date_to"
          class="[&_[role=dialog]]:max-w-[calc(100vw-4rem)]"
      ...
      <template v-else-if="!audit.error.value">
        <div class="divide-y divide-gray-100 xl:hidden">
          <article v-for="log in audit.logs.value" :key="log.id" class="p-4">
            ...
            <div v-if="log.details.length" class="mt-3 space-y-1 text-sm">
              <p
      ...
      >
        ...
        <nav
          ...
          aria-label="Пагинация журнала аудита"
        >
          <UiButton
            size="sm"
            variant="secondary"
            :disabled="audit.page.value === 1 || audit.loading.value"
            @click="audit.load(audit.page.value - 1)"
            >Назад</UiButton
          ><span
            >Страница {{ audit.page.value }} из {{ audit.lastPage.value }}</span
          ><UiButton
            size="sm"
            variant="secondary"
            :disabled="
              audit.page.value === audit.lastPage.value || audit.loading.value
            "
            @click="audit.load(audit.page.value + 1)"
            >Вперёд</UiButton
          >
