...
export function validateXlsxImport(file: File, message = 'Прикрепите XLSX-файл размером не более 10 МБ.'): string {
  return file.name.toLowerCase().endsWith('.xlsx') && file.size <= PRODUCT_IMPORT_MAX_BYTES ? '' : message
}
