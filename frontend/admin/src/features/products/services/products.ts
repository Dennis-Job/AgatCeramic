...
export async function getProductPriceStatusTemplate(): Promise<{
  blob: Blob
  filename: string
}> {
  ...
  return {
    ...
    filename: 'product-price-status-template.xlsx',
  }
}
export async function uploadProductPriceStatusImport(
  file: File,
): Promise<ProductImport> {
  await requestCsrfCookie()
  const body = new FormData()
  body.append('file', file)
  const response = await apiFetch('/admin/products/price-status-import', {
    method: 'POST',
    body,
  })
  if (!response.ok) return fail(response)
...
export async function getProductPriceStatusImport(
  id: number,
): Promise<ProductImport> {
...
export async function getProductPriceStatusImportErrors(
  id: number,
): Promise<{ blob: Blob; filename: string }> {
  const response = await apiFetch(
    `/admin/product-price-status-imports/${id}/errors`,
  )
  if (!response.ok) return fail(response)
  return {
    blob: await response.blob(),
    filename: `product-price-status-import-${id}-errors.xlsx`,
  }
}
export async function getProductGroupImportTemplate(): Promise<{
  blob: Blob
  filename: string
}> {
...
export async function uploadProductGroupImport(
  file: File,
): Promise<ProductImport> {
  await requestCsrfCookie()
  const body = new FormData()
  body.append('file', file)
  const response = await apiFetch('/admin/products/group-import', {
    method: 'POST',
    body,
  })
  if (!response.ok) return fail(response)
...
export async function getProductGroupImport(
  id: number,
): Promise<ProductImport> {
...
export async function getProductGroupImportErrors(
  id: number,
): Promise<{ blob: Blob; filename: string }> {
  ...
  return {
    blob: await response.blob(),
    filename: `product-group-import-${id}-errors.xlsx`,
  }
}
...
export async function getProducts(
  ...
  const query = productQuery(filters, true)
  const response = await apiFetch(
...
export async function getAllProducts(
  filters: Omit<ProductFilters, 'page' | 'perPage'> = {},
): Promise<Product[]> {
  return loadAllPages((request) => getProducts({ ...filters, ...request }))
}
export async function getProductExport(
  ...
  const response = await apiFetch(
    `/admin/products/export${query.size ? `?${query}` : ''}`,
  )
  if (!response.ok) return fail(response)
  const disposition = response.headers.get('Content-Disposition') ?? ''
  const filename =
    disposition.match(/filename="?([^";]+)"?/i)?.[1] ?? 'products.xlsx'
...
export async function getProductImportTemplate(
  ...
  const response = await apiFetch(
    `/admin/products/import-template?category_id=${categoryId}${editing ? '&editing=1' : ''}`,
  )
  if (!response.ok) return fail(response)
  return {
    blob: await response.blob(),
    filename: `products-category-${categoryId}${editing ? '-edit' : ''}.xlsx`,
  }
}
export async function getProductImportErrors(
  id: number,
): Promise<{ blob: Blob; filename: string }> {
  ...
  return {
    blob: await response.blob(),
    filename: `product-import-${id}-errors.xlsx`,
  }
}
