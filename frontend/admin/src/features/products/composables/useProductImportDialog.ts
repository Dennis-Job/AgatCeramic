...
import { validateXlsxImport, validateZipImport } from '../validation/importFiles'
...
export function useProductImportDialog(onCompleted: () => void) {
  ...
  const productImport = useFileImport({
    ...
    validate: file => validateXlsxImport(file, 'Прикрепите файл XLSX размером не более 10 МБ.'),
  ...
  const successful = computed(() => (productImport.result.value?.created_rows ?? 0) + (productImport.result.value?.updated_rows ?? 0))
  const progress = computed(() => productImport.result.value?.total_rows
    ? Math.min(100, Math.round(productImport.result.value.processed_rows / productImport.result.value.total_rows * 100))
    : undefined)
  const imageProgress = computed(() => imageImport.result.value?.total_folders
    ? Math.min(100, Math.round(imageImport.result.value.processed_folders / imageImport.result.value.total_folders * 100))
    : undefined)
  const statusText = computed(() => {
    if (productImport.uploading.value) return 'Загружаем XLSX-файл…'
    if (productImport.result.value?.status === 'pending') return 'Файл ожидает обработки…'
    if (productImport.result.value?.status === 'processing') return productImport.result.value.total_rows
  ...
  async function loadCategories() {
    ...
    try {
      ...
      categoryError.value = reason instanceof Error ? reason.message : 'Не удалось загрузить категории.'
    } finally {
  ...
  async function downloadTemplate(errorsOnly = false) {
    await productImport.download(
      () => errorsOnly && productImport.result.value
        ...
        : getProductImportTemplate(Number(categoryId.value), importMode.value === 'edit'),
      () => errorsOnly
...
function flattenCategories(nodes: Category[], depth = 0): { value: string; label: string }[] {
