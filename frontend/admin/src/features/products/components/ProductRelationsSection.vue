  ...
  >
    ...
    <div class="mt-4 flex flex-col gap-2 lg:flex-row">
      <UiInput
        ...
        aria-label="Поиск сопутствующих товаров"
      /><UiButton
        type="button"
        variant="secondary"
        :loading="saving"
      ...
      ><UiButton
        type="button"
        variant="secondary"
        :disabled="saving || !selectableCandidates.length"
        @click="addRelation"
        ><Plus :size="17" />Добавить</UiButton
      >
    </div>
    <UiEmptyState
    ...
    >
      <div class="grid gap-2 sm:grid-cols-[1fr_160px_90px_auto]">
        <UiSelect
          v-model="relation.related_product_id"
          :options="relationOptions(relation.related_product_id)"
          :accessible-name="`Сопутствующий товар ${index + 1}`"
          searchable
        /><UiSelect
          v-model="relation.type"
          :options="relationTypes"
          :accessible-name="`Тип связи ${index + 1}`"
        /><UiInput
          v-model="relation.sort_order"
          type="number"
          min="0"
