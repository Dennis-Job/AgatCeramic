...
const { saveGroup, copiedFromProduct, groupForm, selectedGroupId, groupOptions, selectGroup, groupErrors, groupSearch, saving, searchGroupProducts, editing, displayAttributeValue, attributes, groupProducts } = useProductEditorContext()
</script>
...
<form id="product-group-form" @submit.prevent="saveGroup">
  ...
  <UiAlert v-if="copiedFromProduct && groupForm.product_ids.includes(copiedFromProduct.id)" class="mt-3 break-words [overflow-wrap:anywhere]" tone="info" live="polite">Исходный товар «{{ copiedFromProduct.name }} · {{ copiedFromProduct.sku }}» уже выбран. <template v-if="selectedGroupId">Новый товар подготовлен для добавления в существующую группу.</template><template v-else>Заполните параметры новой группы и выберите различающуюся характеристику.</template></UiAlert>
  ...
  <div class="mt-4 grid gap-4 sm:grid-cols-2"><UiField label="Название группы" required><UiInput v-model="groupForm.name" class="mt-1.5" required /></UiField><UiField label="Уникальный код" required><UiInput v-model="groupForm.code" class="mt-1.5" required /></UiField></div>
  ...
  <fieldset class="mt-5 rounded-xl border border-gray-300 p-4">
    ...
    <div class="mb-3 flex flex-col gap-2 sm:flex-row"><UiInput v-model="groupSearch" class="min-w-0 flex-1" searchable placeholder="Название или SKU" aria-label="Поиск товаров для группы" /><UiButton type="button" variant="secondary" :loading="saving" :disabled="saving" @click="searchGroupProducts">{{ saving ? 'Поиск…' : 'Найти' }}</UiButton></div><p class="mb-3 text-xs text-gray-500">Первые 25 совпадений той же категории и бренда. Уточните поиск, если товара нет.</p>
