...
export function useAttributeGroupsCatalog() {
  const list = usePaginatedCollection<AttributeGroup>(
  ...
  async function remove(id: number): Promise<void> {
    ...
    await list.reloadAfterDeletion((page) => getAttributeGroups({ page }))
  }
  return {
    groups: list.items,
    ...
    load,
    remove,
    save: (id: number | null, payload: AttributeGroupPayload) =>
      saveAttributeGroup(id, payload),
  }
}
