...
const { groups, pagination, error, loading, load, remove, save } =
  useAttributeGroupsCatalog()
const {
  open,
  title,
  busy: saving,
  ...
  updateSlug,
  submit,
} = useAttributeGroupForm(save, load)
...
<template>
  <section class="mx-auto admin-page" :aria-busy="loading">
    ...
    /><ConfirmDialog
      ...
      :description="`Группа «${deleting?.name ?? ''}» будет удалена. Это действие нельзя отменить.`"
      :busy="deletingBusy"
      @close="deleting = null"
