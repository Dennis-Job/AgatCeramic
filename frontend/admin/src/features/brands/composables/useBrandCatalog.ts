...
export function useBrandCatalog() {
  ...
  async function load(
    ...
    await list.load(page, (requestedPage) => getBrands({ page: requestedPage }))
  }
  async function loadAfterDeletion(): Promise<void> {
    await list.reloadAfterDeletion((page) => getBrands({ page }))
  }
  return {
    brands: list.items,
    ...
    load,
    loadAfterDeletion,
    saveBrand: (id: number | null, payload: BrandPayload) =>
      saveBrand(id, payload),
