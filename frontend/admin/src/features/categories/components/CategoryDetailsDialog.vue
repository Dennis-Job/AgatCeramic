...
const filterAttributes = () =>
  props.attributes.filter((attribute) => attribute.is_filterable)
</script>
<template>
  <UiDialog
    :open="Boolean(category)"
    labelledby="category-details-title"
    ...
    ><template v-if="category"
      ...
      <p
        v-if="loading"
        class="mt-6 py-8 text-center text-sm text-gray-500"
        role="status"
      >
      ...
      <template v-else
        ...
        <section class="mt-5">
          ...
          <p v-else class="mt-2 text-sm text-gray-500">
            Характеристики для фильтрации не назначены.
          </p>
        </section>
        <section class="mt-5">
          ...
          <div v-if="attributes.length" class="mt-2 flex flex-wrap gap-2">
            <UiBadge
              ...
              >{{ attribute.name
                ...
                ({{ attribute.unit }})</template
              ><template v-if="attribute.is_required">
                · обязательная</template
              ></UiBadge
            >
          </div>
          <p v-else class="mt-2 text-sm text-gray-500">Не назначены.</p>
        </section></template
      ></template
    ></UiDialog
  >
