...
export function useCategoryAssignments() {
  ...
  async function show(value: Category): Promise<void> {
    ...
    try {
      const [
        ...
        getCategoryAttributes(value.id),
        getCategoryAttributeGroups(value.id),
      ])
      attributes.value = availableAttributes
      groups.value = availableGroups
      selectedAttributeIds.value = assignedAttributes.map((item) => item.id)
      requiredAttributeIds.value = assignedAttributes
        .filter((item) => item.is_required)
        .map((item) => item.id)
      selectedGroupIds.value = assignedGroups.map((item) => item.id)
    } catch (reason) {
      error.value =
  ...
  function setGroupIds(ids: Array<number | string>): void {
    const previous = selectedGroupIds.value
    selectedGroupIds.value = ids.map(Number)
    const removed = previous.filter(
      (id) => !selectedGroupIds.value.includes(id),
    )
    if (removed.length)
      selectedAttributeIds.value = selectedAttributeIds.value.filter(
        (id) =>
          !removed.includes(
            attributes.value.find((item) => item.id === id)
              ?.attribute_group_id ?? -1,
          ),
      )
    requiredAttributeIds.value = requiredAttributeIds.value.filter((id) =>
      selectedAttributeIds.value.includes(id),
    )
  }
  function setAttributeIds(ids: Array<number | string>): void {
  ...
  async function submit(): Promise<void> {
    ...
    try {
      await replaceCategoryAttributeGroups(
        category.value.id,
        selectedGroupIds.value.map((id, sort_order) => ({ id, sort_order })),
      )
      await replaceCategoryAttributes(
        category.value.id,
        selectedAttributeIds.value.map((id, sort_order) => ({
          id,
          sort_order,
          is_required: requiredAttributeIds.value.includes(id),
        })),
      )
      open.value = false
    } catch (reason) {
      error.value =
        reason instanceof Error
          ? reason.message
          : 'Не удалось сохранить характеристики категории.'
    } finally {
      busy.value = false
    }
  }
