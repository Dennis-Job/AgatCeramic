...
<template>
  <section class="mx-auto admin-page">
    ...
    <UiCard :padded="false" class="overflow-hidden">
      ...
      >
        ...
        <UiSelect
          ...
          accessible-name="Фильтр по статусу"
          :options="statusFilterOptions"
          @change="workspace.load()"
        />
        <UiButton type="submit" variant="secondary" class="w-full sm:w-auto"
          >Найти</UiButton
        >
      </form>
      <EmployeesList
        v-if="!workspace.error.value"
        :employees="workspace.employees.value"
        :loading="workspace.loading.value"
        :can-manage="workspace.canManage.value"
        :can-edit="workspace.canEdit.value"
    ...
    <EmployeeFormDialog
      ...
      :roles="workspace.roles.value"
      :busy="workspace.saving.value"
      :error="workspace.formError.value"
      @close="workspace.closeForm"
      @submit="workspace.submit"
    />
    <ConfirmDialog
      :open="Boolean(workspace.deleting.value)"
      title="Удалить сотрудника?"
      :description="`Учётная запись «${workspace.deleting.value?.name ?? ''}» будет удалена. Это действие нельзя отменить.`"
