  ...
  <template v-else>
    <div class="divide-y divide-gray-100 xl:hidden">
      <article v-for="employee in employees" :key="employee.id" class="p-4">
        ...
        <div class="mt-3 flex flex-wrap gap-1"><UiBadge v-for="role in employee.roles" :key="role.id" tone="primary">{{ role.name }}</UiBadge><span v-if="!employee.roles.length" class="text-sm text-gray-400">Роли не назначены</span></div>
