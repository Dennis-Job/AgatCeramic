...
export function useRolesWorkspace() {
  ...
  const title = computed(() =>
    editing.value ? `Роль: ${editing.value.name}` : 'Новая роль',
  )
  ...
  watch(
    ...
    (name) => {
      if (
        !editing.value &&
        (form.value.slug === '' || form.value.slug === generatedSlug.value)
      ) {
  ...
  async function save(): Promise<void> {
    ...
    formError.value = ''
    try {
      await saveRole(editing.value?.id ?? null, form.value)
      opened.value = false
      await load()
    } catch (reason) {
      formError.value =
        reason instanceof Error ? reason.message : 'Не удалось сохранить роль.'
    } finally {
      saving.value = false
    }
  }
  ...
  async function remove(): Promise<void> {
    ...
    deleteError.value = ''
    try {
      await deleteRole(deleting.value.id)
      deleting.value = null
      await load()
    } catch (reason) {
      deleteError.value =
        reason instanceof Error ? reason.message : 'Не удалось удалить роль.'
    } finally {
