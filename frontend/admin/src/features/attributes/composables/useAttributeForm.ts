...
export function useAttributeForm(
  ...
  function updateType(value: AttributeType): void {
    form.value.type = value
    if (
      ['select', 'multiselect'].includes(value) &&
      form.value.options.length === 0
    )
      form.value.options.push({ value: '', label: '', sort_order: 0 })
  }
  async function submit(): Promise<void> {
    error.value = validateAttribute(form.value)
    if (error.value) return
    busy.value = true
    try {
      await saveAttribute(editing.value?.id ?? null, form.value)
      open.value = false
      await onSaved()
    } catch (reason) {
      error.value =
