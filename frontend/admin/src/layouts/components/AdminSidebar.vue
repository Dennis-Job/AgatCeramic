...
<template>
  ...
  >
    <div class="mb-9 flex items-center justify-between px-2">
      <RouterLink class="flex min-w-0 items-center gap-3" to="/" @click="close">
        <span class="grid h-10 w-10 shrink-0 place-items-center"
          ><img src="/logo.svg" alt="AgatCeramic" class="logo-primary"
        /></span>
        <span class="min-w-0"
          ><span
            class="block truncate text-2xl leading-6 font-bold tracking-tight text-gray-800"
            >Agat<span class="text-gray-400">Ceramic</span></span
          ><span class="block text-end text-xs">Админ-панель</span></span
        >
      </RouterLink>
      <button
        ref="closeButton"
        type="button"
        class="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-gray-500 transition hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 lg:hidden"
    ...
    <template v-if="visibleProductManagementNavigation.length"
      ...
      <nav class="space-y-1" aria-label="Управление товарами">
        <RouterLink
    ...
    <template v-if="visibleEmployeeNavigation.length"
      ...
      <nav class="space-y-1" aria-label="Управление сотрудниками">
        <RouterLink
    ...
    <nav class="space-y-1" aria-label="Разработка">
      <RouterLink
        v-for="item in developmentNavigation"
        :key="item.to"
        :to="item.to"
        class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-600 transition hover:bg-gray-25"
        active-class="!bg-primary-50 !text-primary-600"
