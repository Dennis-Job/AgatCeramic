...
import { attributeTypeLabel, attributeTypeOptions } from '../utils/attributeTypes'

const types = attributeTypeOptions
...
function emptyForm(): AttributePayload { return { attribute_group_id: null, name: '', slug: '', type: 'string', unit: null, is_filterable: false, is_visible_on_product_page: true, sort_order: 0, options: [] } }
...
<template>
  <section class="mx-auto admin-page" :aria-busy="loading">
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-card">
      <CollectionLoadingState v-if="loading" label="Загрузка характеристик…" />
      <div v-else-if="attributes.length" class="divide-y divide-gray-100"><article v-for="attribute in attributes" :key="attribute.id" class="flex flex-wrap items-start gap-3 p-4 sm:flex-nowrap sm:items-center sm:gap-4"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-primary-50 text-primary-600"><ListFilter :size="20" /></span><div class="min-w-0 flex-[1_1_calc(100%-52px)] sm:flex-1"><div class="flex flex-wrap items-center gap-x-1.5 gap-y-1"><h2 class="font-semibold text-gray-800">{{ attribute.name }}<span v-if="attribute.unit" class="font-medium text-gray-500"> ({{ attribute.unit }})</span></h2><span v-if="groupName(attribute)" class="rounded-full bg-blue-light-50 px-2 py-0.5 text-xs font-medium text-blue-light-500">Группа: {{ groupName(attribute) }}</span><span v-if="attribute.is_filterable" class="rounded-full bg-success-50 px-2 py-0.5 text-xs font-medium text-success-500">В фильтрах</span><span v-if="attribute.is_visible_on_product_page" class="rounded-full bg-primary-50 px-2 py-0.5 text-xs font-medium text-primary-600">На странице товара</span></div><p class="mt-0.5 text-sm text-gray-500">{{ attributeTypeLabel(attribute.type) }} · /{{ attribute.slug }} · значений: {{ attribute.options.length }}</p></div><div v-if="canManage" class="ml-[52px] flex gap-1 sm:ml-0"><button class="rounded-lg p-2 text-gray-500 hover:bg-primary-50 hover:text-primary-600" :aria-label="`Редактировать характеристику ${attribute.name}`" @click="open(attribute)"><Pencil :size="17" /></button><button class="rounded-lg p-2 text-gray-500 hover:bg-error-50 hover:text-error-500" :aria-label="`Удалить характеристику ${attribute.name}`" @click="deleting = attribute"><Trash2 :size="17" /></button></div></article></div>
    ...
    <BaseDialog :open="opened" labelledby="attribute-dialog-title" describedby="attribute-dialog-description" :close-disabled="isSaving" panel-class="w-full max-w-xl" @close="closeDialog">
      <form class="max-h-[90vh] w-full overflow-y-auto rounded-xl bg-white p-6 shadow-xl" @submit.prevent="save">
        ...
        <div class="mt-6 grid gap-4">
          ...
          <div class="grid gap-2 sm:grid-cols-2"><BaseCheckbox mode="boolean" :checked="form.is_filterable" @update:checked="form.is_filterable = $event">Использовать в фильтре</BaseCheckbox><BaseCheckbox mode="boolean" :checked="form.is_visible_on_product_page" @update:checked="form.is_visible_on_product_page = $event">Показывать на странице товара</BaseCheckbox></div>
