...
<template>
  <section class="mx-auto max-w-[1440px]">
    <div class="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p class="text-sm font-medium text-gray-500">Управление доступом</p><h1 class="mt-1 text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Сотрудники</h1></div><button class="inline-flex items-center gap-2 rounded-lg bg-primary-500 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-primary-600" @click="openForm()"><Plus :size="18" />Добавить сотрудника</button></div>
    <p v-if="error" class="mb-4 rounded-lg border border-error-200 bg-error-50 px-4 py-3 text-sm text-error-500">{{ error }}</p>
    <div class="rounded-xl border border-gray-200 bg-white shadow-card"><div class="grid grid-cols-2 gap-3 border-b border-gray-100 p-5 sm:grid-cols-[minmax(0,1fr)_180px_auto]"><BaseInput v-model="search" class="col-span-2 sm:col-auto" searchable placeholder="Имя или email" @keyup.enter="load" /><BaseSelect v-model="status" accessible-name="Фильтр по статусу" :options="statusFilterOptions" @change="load" /><button class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-semibold text-gray-600 sm:w-auto" @click="load">Найти</button></div>
      <div class="overflow-x-auto"><table class="w-full min-w-[780px] text-left text-sm"><thead class="bg-gray-25 text-xs font-medium text-gray-500"><tr><th class="px-6 py-3">Сотрудник</th><th class="px-6 py-3">Роли</th><th class="px-6 py-3">Статус</th><th class="px-6 py-3">Последний вход</th><th class="px-6 py-3" /></tr></thead><tbody><tr v-if="loading"><td class="px-6 py-6 text-gray-500" colspan="5">Загрузка…</td></tr><tr v-else-if="!employees.length"><td class="px-6 py-6 text-gray-500" colspan="5">Сотрудники не найдены.</td></tr><tr v-for="employee in employees" :key="employee.id" class="border-t border-gray-100 text-gray-600"><td class="px-6 py-4"><p class="font-semibold text-gray-700">{{ employee.name }}</p><p class="mt-0.5 text-xs text-gray-500">{{ employee.email }}</p></td><td class="px-6 py-4"><span v-for="role in employee.roles" :key="role.id" class="mr-1 inline-block rounded-full bg-primary-50 px-2 py-1 text-xs font-medium text-primary-600">{{ roleDisplayName(role) }}</span></td><td class="px-6 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-medium" :class="employee.status === 'active' ? 'bg-success-50 text-success-700' : 'bg-error-50 text-error-500'">{{ employee.status === 'active' ? 'Активен' : 'Заблокирован' }}</span></td><td class="px-6 py-4 text-gray-500">{{ employee.last_login_at ? new Date(employee.last_login_at).toLocaleString('ru-RU') : '—' }}</td><td class="px-6 py-4"><div class="flex justify-end gap-2"><button class="rounded-lg p-2 text-gray-500 hover:bg-primary-50 hover:text-primary-600" aria-label="Редактировать" @click="openForm(employee)"><Pencil :size="17" /></button><button class="rounded-lg p-2 text-gray-500 hover:bg-error-50 hover:text-error-500" aria-label="Удалить" @click="requestRemoval(employee)"><Trash2 :size="17" /></button></div></td></tr></tbody></table></div>
    </div>
    <div v-if="isFormOpen" class="fixed inset-0 z-50 grid place-items-center bg-gray-900/50 p-4" @click.self="isFormOpen = false"><form class="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-xl bg-white p-6 shadow-xl" @submit.prevent="submit"><div class="flex items-start justify-between"><div><h2 class="text-lg font-bold text-gray-900">{{ title }}</h2><p class="mt-1 text-sm text-gray-500">Укажите учётные данные, статус и роли.</p></div><button type="button" class="rounded-lg p-1 text-gray-500" @click="isFormOpen = false"><X :size="20" /></button></div><div class="mt-6 grid gap-4"><label class="text-sm font-medium text-gray-700">Имя<BaseInput v-model="form.name" class="mt-1.5 w-full font-normal" required /></label><label class="text-sm font-medium text-gray-700">Email<BaseInput v-model="form.email" class="mt-1.5 w-full font-normal" required type="email" /></label><div class="grid gap-4 sm:grid-cols-2"><label class="text-sm font-medium text-gray-700">{{ editing ? 'Новый пароль (необязательно)' : 'Пароль' }}<BaseInput v-model="form.password" class="mt-1.5 w-full font-normal" :required="!editing" minlength="12" type="password" /></label><label class="text-sm font-medium text-gray-700">Подтверждение пароля<BaseInput v-model="form.password_confirmation" class="mt-1.5 w-full font-normal" :required="!editing || Boolean(form.password)" type="password" /></label></div><label class="text-sm font-medium text-gray-700">Статус<BaseSelect v-model="form.status" class="mt-1.5 w-full font-normal" accessible-name="Статус сотрудника" :options="statusOptions" /></label><fieldset><legend class="text-sm font-medium text-gray-700">Роли</legend><div class="mt-2 grid gap-2 sm:grid-cols-2"><BaseCheckbox v-for="role in roles" :key="role.id" v-model="form.role_ids" :value="role.id">{{ roleDisplayName(role) }}</BaseCheckbox></div></fieldset></div><div class="mt-6 flex justify-end gap-3"><button type="button" class="rounded-lg px-4 py-2.5 text-sm font-semibold text-gray-600" @click="isFormOpen = false">Отмена</button><button class="rounded-lg bg-primary-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary-600">Сохранить</button></div></form></div>
    <div v-if="deleting" class="fixed inset-0 z-50 grid place-items-center bg-gray-900/50 p-4" @click.self="!isDeleting && (deleting = null)"><section class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" role="dialog" aria-modal="true" aria-labelledby="delete-employee-title"><h2 id="delete-employee-title" class="text-lg font-bold text-gray-900">Удалить сотрудника?</h2><p class="mt-3 text-sm leading-6 text-gray-500">Учётная запись «{{ deleting.name }}» будет удалена. Это действие нельзя отменить.</p><div class="mt-6 flex justify-end gap-3"><button type="button" class="rounded-lg px-4 py-2.5 text-sm font-semibold text-gray-600 disabled:cursor-not-allowed disabled:opacity-60" :disabled="isDeleting" @click="deleting = null">Отмена</button><button type="button" class="rounded-lg bg-error-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-error-700 disabled:cursor-not-allowed disabled:opacity-60" :disabled="isDeleting" @click="remove">{{ isDeleting ? 'Удаление…' : 'Удалить' }}</button></div></section></div>
  </section>
</template>
