...
<template>
  <section class="mx-auto max-w-[1440px]">
    ...
    <div v-if="deleting" class="fixed inset-0 z-50 grid place-items-center bg-[#101828]/50 p-4" @click.self="!isDeleting && (deleting = null)"><section class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" role="dialog" aria-modal="true" aria-labelledby="delete-employee-title"><h2 id="delete-employee-title" class="text-lg font-bold text-[#101828]">Удалить сотрудника?</h2><p class="mt-3 text-sm leading-6 text-[#667085]">Учётная запись «{{ deleting.name }}» будет удалена. Это действие нельзя отменить.</p><div class="mt-6 flex justify-end gap-3"><button type="button" class="rounded-lg px-4 py-2.5 text-sm font-semibold text-[#475467] disabled:cursor-not-allowed disabled:opacity-60" :disabled="isDeleting" @click="deleting = null">Отмена</button><button type="button" class="rounded-lg bg-[#b42318] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#912018] disabled:cursor-not-allowed disabled:opacity-60" :disabled="isDeleting" @click="remove">{{ isDeleting ? 'Удаление…' : 'Удалить' }}</button></div></section></div>
  </section>
</template>
