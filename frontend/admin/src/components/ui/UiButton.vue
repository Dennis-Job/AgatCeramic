...
const props = withDefaults(defineProps<{ variant?: 'primary' | 'secondary' | 'danger' | 'ghost' | 'danger-ghost'; size?: 'sm' | 'md' | 'lg'; loading?: boolean }>(), { variant: 'primary', size: 'md', loading: false })
...
const classes = computed(() => [
  ...
  props.variant === 'primary' ? 'bg-primary-500 text-white hover:bg-primary-600' : props.variant === 'danger' ? 'bg-error-500 text-white hover:bg-error-700' : props.variant === 'danger-ghost' ? 'text-error-500 hover:bg-error-50 hover:text-error-700' : props.variant === 'secondary' ? 'border border-gray-300 text-gray-600 hover:border-primary-200 hover:bg-primary-50 hover:text-primary-600' : 'text-gray-600 hover:bg-gray-50',
  attrs.class,
])
