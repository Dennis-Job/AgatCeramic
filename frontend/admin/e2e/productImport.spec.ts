...
async function openImport(page: Page) {
  ...
  await page
    .getByRole('button', { name: 'Загрузить массово', exact: true })
    .click()
...
test('product Excel import downloads category template and preserves processing when closed', async ({
  page,
}) => {
...
test('product Excel import errors are downloadable and modal is accessible at supported widths', async ({
  page,
}, testInfo) => {
  ...
  await expect(
    dialog.getByText('Товар с таким наименованием уже существует.'),
  ).toBeVisible()
  const download = page.waitForEvent('download')
  ...
  for (const width of [320, 640, 768, 1024, 1280]) {
    ...
    await dialog.screenshot({
      path: testInfo.outputPath(`modal-errors-${width}.png`),
    })
  }
  ...
  await expect(
    dialog.getByRole('tab', { name: 'Загрузка изображений' }),
  ).toBeFocused()
  await expect(
    dialog.getByRole('heading', { name: 'Подготовьте ZIP-архив' }),
  ).toBeVisible()
  await expect(
    dialog.getByRole('button', { name: 'Выбрать ZIP-архив' }),
  ).toBeVisible()
  await page.keyboard.press('Home')
  await expect(
    dialog.getByRole('tab', { name: 'Загрузка товаров' }),
  ).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(
    page.getByRole('button', { name: 'Загрузить массово', exact: true }),
  ).toBeFocused()
})
...
test('category edit template includes SKU and uses the category import flow', async ({
  ...
  await dialog
    ...
    .click()
  await expect(
    dialog.getByText('SKU определяет редактируемый товар', { exact: false }),
  ).toBeVisible()
  const templateRequest = page.waitForRequest((request) =>
...
test('product image ZIP import uploads, polls and downloads its accessible responsive error report', async ({
  ...
  await expect(
    dialog.getByText('Папок обработано').locator('..'),
...
test('variation-group Excel import downloads, uploads and reports its accessible result', async ({
  ...
  await page.goto('/products')
  await page
    .getByRole('button', { name: 'Группы вариантов', exact: true })
    .click()
  ...
  expect((await uploadRequest).method()).toBe('POST')
  await expect(
    dialog.getByText('Обработано групп: 2 из 2. Изменено: 1. Ошибок: 1.'),
  ).toBeVisible()
  for (const width of [320, 640, 768, 1024, 1280]) {
    await page.setViewportSize({ width, height: 800 })
    expect(
...
test('price and status Excel import preserves its contract and responsive accessible states', async ({
  ...
  await page.goto('/products')
  await page
    .getByRole('button', { name: 'Цены и статусы', exact: true })
    .click()
  ...
  expect((await uploadRequest).method()).toBe('POST')
  await expect(
    dialog.getByText('Обработано: 3 из 3. Изменено: 2. Ошибок: 1.'),
  ).toBeVisible()
  ...
  await page.keyboard.press('Escape')
  await expect(
    page.getByRole('button', { name: 'Цены и статусы', exact: true }),
  ).toBeFocused()
})
