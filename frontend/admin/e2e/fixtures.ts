...
function observeBrowserIssues(page: Page, issues: BrowserIssue[]): void {
  page.on('console', (message: ConsoleMessage) => {
    ...
    const isUnresolvedVueComponent =
  ...
  browserIssueGuard: [
    async ({ context }, use) => {
      ...
      const unexpectedIssues = issues.filter(
        (issue) => !isExpectedApiError(issue, expectedApiErrors),
      )
      baseExpect(
        unexpectedIssues,
        'Browser console/runtime errors must be resolved or explicitly expected',
      ).toEqual([])
    },
    { auto: true },
  ],
