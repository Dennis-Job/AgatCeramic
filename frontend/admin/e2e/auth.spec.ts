...
async function mockAuthApi(
  page: Page,
  options: { loginGate?: Promise<void>; failLogin?: boolean } = {},
): Promise<void> {
  ...
  await page.route('**/api/v1/admin/auth/**', async (route) => {
    ...
    if (path === '/api/v1/admin/auth/login') {
      ...
      if (options.failLogin) {
        await route.fulfill({
          status: 422,
          json: { error: { message: 'Неверный email или пароль.' } },
        })
        return
      }
      authenticated = true
...
test('login preserves credentials contract, autocomplete, keyboard flow and redirect', async ({
  page,
  browserIssueGuard,
}) => {
  ...
  const loginRequest = page.waitForRequest((request) =>
    new URL(request.url()).pathname.endsWith('/admin/auth/login'),
  )
  ...
  await expect(
    page.getByRole('button', { name: 'Выполняется вход…' }),
  ).toBeDisabled()
  releaseLogin()
  await expect(page).toHaveURL('/')
})

test('login exposes a retryable API error state', async ({
  page,
  browserIssueGuard,
}) => {
  browserIssueGuard.allowApiError(401, '/api/v1/admin/auth/me')
...
test('forgot-password preserves API contract and success state', async ({
  page,
  browserIssueGuard,
}) => {
  browserIssueGuard.allowApiError(401, '/api/v1/admin/auth/me')
  ...
  const resetRequest = page.waitForRequest((request) =>
    new URL(request.url()).pathname.endsWith('/admin/auth/forgot-password'),
  )
  ...
  await expect(
    page.getByRole('button', { name: 'Отправить ссылку' }),
  ).toBeDisabled()
})

test('reset-password preserves token contract, autocomplete and login success redirect', async ({
  page,
  browserIssueGuard,
}) => {
  browserIssueGuard.allowApiError(401, '/api/v1/admin/auth/me')
  await mockAuthApi(page)
  await page.goto(
    '/reset-password?email=employee%40example.test&token=reset-token',
  )
  ...
  const resetRequest = page.waitForRequest((request) =>
    new URL(request.url()).pathname.endsWith('/admin/auth/reset-password'),
  )
