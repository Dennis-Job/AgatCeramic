...
class ProductPriceStatusImportTest extends TestCase
{
    ...
    {
        $file = app(ProductPriceStatusImportService::class)->createTemplate(); $reader = new Reader; $reader->open($file['path']);
        $this->assertSame(['Цены', 'Активность', 'Распродажа'], collect($reader->getSheetIterator())->map->getName()->values()->all());
        $reader->close();
        $zip = new ZipArchive; $zip->open($file['path']);
    ...
    {
        Storage::fake('local'); $actor = $this->actor();
        ...
        $diskPath = 'product-price-status-imports/test.xlsx'; Storage::disk('local')->put($diskPath, file_get_contents($path)); @unlink($path);
        ...
        $product->refresh(); $import->refresh();
        $this->assertSame('80.00', $product->price); $this->assertSame('120.00', $product->old_price); $this->assertFalse($product->is_active); $this->assertTrue($product->is_on_sale); $this->assertSame('Не менять', $product->name);
        $this->assertSame('completed', $import->status); $this->assertSame(1, $import->failed_rows); $this->assertSame(3, $import->updated_rows);
    ...
    {
        $path = tempnam(sys_get_temp_dir(), 'price-status-'); $writer = new Writer; $writer->openToFile($path); $first = true;
        foreach ($sheets as $name => $rows) { if (! $first) $writer->addNewSheetAndMakeItCurrent(); $first = false; $writer->getCurrentSheet()->setName($name); foreach ($rows as $values) $writer->addRow(Row::fromValues($values)); }
        $writer->close(); return $path;
    }

    private function actor(): User
    {
        $this->seed([RoleSeeder::class, PermissionSeeder::class]); $role = Role::factory()->create(); $role->permissions()->attach(Permission::query()->where('code', 'imports.manage')->sole()); $user = User::factory()->create(); $user->roles()->attach($role); return $user;
    }
}
